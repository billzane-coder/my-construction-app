...
import { 
  Plus, Minus, Users, HardHat, Building2, FileCheck, ShieldCheck, 
  FileText, Phone, Mail, ChevronLeft, Loader2, Upload, 
...
export default function ProjectWarRoom() {
  ...
  if (loading) return <div className="min-h-screen bg-slate-950 flex items-center justify-center text-blue-500 font-black animate-pulse uppercase">Syncing Master Vault...</div>
    ...
    <div className="max-w-7xl mx-auto p-4 md:p-8 bg-slate-950 min-h-screen font-sans text-slate-100 pb-20">
      ...
      <div className="mb-12 border-b-4 border-blue-600 pb-8 flex flex-col xl:flex-row justify-between items-start xl:items-end gap-8">
        <div className="space-y-1">
          <button onClick={() => router.push('/projects')} className="text-[10px] font-black uppercase text-slate-500 mb-2 hover:text-white flex items-center gap-1">
        ...
        <div className="flex flex-wrap gap-4 items-center">
          <div className="flex items-center gap-3 bg-blue-600/10 border border-blue-500/20 px-6 py-4 rounded-3xl">
            ...
            <div className="text-left">
              <p className="text-[8px] font-black text-slate-500 uppercase">Headcount</p>
      ...
      {/* TAB 2: BLUEPRINT VAULT (Linking to Internal Viewer) */}
      {activeTab === 'plans' && (
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 animate-in fade-in duration-500">
          ...
          {docs.filter(d => d.doc_type === 'Plan').map(plan => (
            <div key={plan.id} className="bg-slate-900 p-8 rounded-[48px] border border-slate-800 shadow-2xl relative">
              ...
              <Link href={`/projects/${id}/viewer/${plan.id}`} className="block w-full text-center bg-slate-800 hover:bg-blue-600 py-6 rounded-3xl text-[11px] font-black uppercase text-white transition-all">
      ...
      {/* TAB 3: TRADE COMPLIANCE VAULT (3-Box System) */}
      {activeTab === 'contacts' && (
              ...
              <div key={trade.id} className="bg-slate-900 rounded-[48px] border border-slate-800 shadow-2xl overflow-hidden flex flex-col">
                <div className="p-8 md:p-10 border-b border-slate-800 bg-slate-900/50">
                  {editingContact?.id === trade.id ? (
                    <form onSubmit={handleUpdateContact} className="grid grid-cols-1 md:grid-cols-3 gap-6">
                      <input className="bg-slate-950 p-4 rounded-xl border border-blue-500/50 text-white font-bold" value={editingContact.company} onChange={e => setEditingContact({...editingContact, company: e.target.value})} />
                      <input className="bg-slate-950 p-4 rounded-xl border border-slate-800 text-white font-bold" value={editingContact.trade_role} onChange={e => setEditingContact({...editingContact, trade_role: e.target.value})} />
                      <div className="flex gap-2">
                        <button type="submit" className="flex-1 bg-blue-600 p-4 rounded-xl font-black text-white uppercase text-[10px]"><Save size={16}/></button>
                        <button onClick={() => setEditingContact(null)} className="flex-1 bg-slate-800 p-4 rounded-xl font-black text-white uppercase text-[10px]"><X size={16}/></button>
                      </div>
                    </form>
                  ) : (
                    <div className="flex justify-between items-start">
                      ...
                      <button onClick={() => setEditingContact(trade)} className="p-4 bg-slate-800 rounded-2xl text-slate-400 hover:text-white transition-all">
                        <Settings2 size={20} />
                      </button>
                    </div>
                  )}
      ...
      {showSubmittalModal.show && (
        <div className="fixed inset-0 bg-slate-950/95 z-[100] flex items-center justify-center p-4 backdrop-blur-md">
          <div className="bg-slate-900 border-2 border-blue-600 p-8 rounded-[40px] max-w-lg w-full space-y-6 shadow-2xl">
            ...
            <input id="docTitle" placeholder="Document Name (e.g. WSIB Clearance)" className="w-full p-4 bg-slate-950 border border-slate-800 rounded-xl font-bold text-white outline-none focus:border-blue-500" />
            <input type="file" id="docFile" accept=".pdf" className="w-full text-xs text-slate-500 file:bg-slate-800 file:text-white file:px-4 file:py-2 file:rounded-lg" />
            <div className="flex gap-4">
              <button onClick={() => setShowSubmittalModal({show: false, contactId: null, category: 'Submittal'})} className="flex-1 bg-slate-800 py-4 rounded-2xl font-black text-white uppercase text-[10px]">Cancel</button>
              <button onClick={() => {
                  const title = (document.getElementById('docTitle') as HTMLInputElement).value
                  const file = (document.getElementById('docFile') as HTMLInputElement).files?.[0]
                  if(file && title) handleUploadDoc(file, showSubmittalModal.contactId!, showSubmittalModal.category, title)
                }} className="flex-1 bg-blue-600 py-4 rounded-2xl font-black text-white uppercase text-[10px]">
