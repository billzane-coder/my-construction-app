...
export default function InspectionMatrix() {
  ...
  const fetchData = async () => {
    ...
    if (insData.data) {
      ...
      const uniqueUnits = Array.from(new Set(insData.data.map(i => i.unit_name))).sort()
  ...
  const handleExportPDF = async () => {
    ...
    try {
      ...
      if (requestMode) {
        ...
        // ... (Logic for full matrix landscape export remains same as before)
      ...
      // 2. AUTO-ARCHIVE TO SUPABASE (If in Batch Request mode)
      if (requestMode) {
        ...
        if (!uploadError) {
          ...
          // Update all selected database records with this specific PDF link
          const updates = selectedRequests.map(req => {
            const [unit, phase] = req.split('|')
            return supabase.from('project_inspections')
              .update({ 
                status: 'Requested', 
                document_url: publicUrl // This archives the copy to the lot
              })
              .eq('project_id', id)
              .eq('unit_name', unit)
