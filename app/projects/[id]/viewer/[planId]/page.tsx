...
export default function ProPlanViewer() {
  ...
  // New States for Views & Tools
  const [activeTool, setActiveTool] = useState<Tool>('select')
  const [viewMode, setViewMode] = useState<'clean' | 'marked'>('clean') // Default to Clean
  const [loading, setLoading] = useState(true)
  
  // High-Res Zoom (Now allows 50%)
  ...
  // --- INIT FETCH & SIBLING REVISIONS ---
  useEffect(() => {
    async function init() {
      // 1. Get current plan
      const { data: p } = await supabase.from('project_documents').select('*').eq('id', planId).single()
      
      // 2. Fetch all versions of this plan (matching title) and the markups for the current one
  ...
  const handlePanStart = (e: React.MouseEvent | React.TouchEvent) => {
    ...
    if ((e.target as Element).tagName === 'svg' || (e.target as Element).tagName === 'DIV') {
      setIsPanning(true)
  ...
  const handleStageDown = (e: React.MouseEvent | React.TouchEvent) => {
    if (viewMode === 'clean') return // Disable drawing in clean mode
  ...
  return (
    <div className="h-screen w-screen bg-slate-900 flex flex-col overflow-hidden print:bg-white">
      ...
      <div className="bg-slate-950 border-b border-slate-800 p-4 flex flex-wrap justify-between items-center z-50 shadow-2xl print:hidden gap-4">
        ...
        <div className="flex gap-4 items-center">
          ...
          <select 
            ...
            onChange={(e) => router.push(`/projects/${id}/viewer/${e.target.value}`)}
            className="bg-slate-900 border border-slate-700 text-white font-black text-[10px] uppercase px-4 py-2 rounded-xl outline-none"
          >
            {planVersions.map(v => (
              <option key={v.id} value={v.id}>{plan?.title} - {v.revision_number}</option>
            ))}
          </select>
          ...
          <select 
            ...
            className={`font-black text-[10px] uppercase px-4 py-2 rounded-xl outline-none border ${viewMode === 'marked' ? 'bg-amber-500/10 text-amber-500 border-amber-500/30' : 'bg-emerald-500/10 text-emerald-500 border-emerald-500/30'}`}
          >
        ...
        {/* Center: Tools (Only visible if viewing markups) */}
        {viewMode === 'marked' && (
          <div className="flex gap-2 items-center bg-slate-900 p-1 rounded-2xl border border-slate-800">
            ...
            {/* Delete Tool appears when something is selected */}
            {selectedId && (
        ...
        <div className="flex gap-3 items-center">
          <button onClick={() => window.print()} className="bg-slate-800 hover:bg-slate-700 text-white px-4 py-2 rounded-xl text-[10px] font-black uppercase transition-all">
            Export PDF
          </button>
          
          <div className="h-6 w-[1px] bg-slate-800 mx-1" />

          {/* Zoom allows down to 0.5 (50%) */}
          <button onClick={() => setScale(s => s + 0.25)} className="bg-slate-800 text-white w-8 h-8 rounded-lg font-black">+</button>
          <span className="text-[10px] font-black text-slate-500 w-10 text-center">{Math.round(scale * 100)}%</span>
          <button onClick={() => setScale(s => Math.max(0.5, s - 0.25))} className="bg-slate-800 text-white w-8 h-8 rounded-lg font-black">-</button>
        </div>
      </div>
      ...
      <div className="flex-1 relative overflow-hidden bg-slate-800 select-none print:bg-white print:overflow-visible">
        <div 
          className="absolute transition-transform duration-75 origin-top-left print:relative print:transform-none print:w-full"
          ...
          <div ref={containerRef} className="relative inline-block shadow-[0_0_100px_rgba(0,0,0,0.5)] bg-white print:shadow-none print:w-full">
            ...
            {plan?.file_url && (
              <Document file={plan.file_url} loading={<div className="p-20 text-slate-500 font-black">Rendering Document...</div>}>
                <Page pageNumber={1} scale={scale} renderTextLayer={false} renderAnnotationLayer={false} className="pointer-events-none print:w-full" />
              </Document>
            )}

            {/* SVG EDIT LAYER (Only renders if viewMode is 'marked' OR we are printing) */}
            {(viewMode === 'marked' || typeof window !== 'undefined' && window.matchMedia('print').matches) && (
              <svg 
                className={`absolute inset-0 w-full h-full z-10 ${activeTool === 'select' ? (isPanning ? 'cursor-grabbing' : 'cursor-grab') : 'cursor-crosshair'}`}
