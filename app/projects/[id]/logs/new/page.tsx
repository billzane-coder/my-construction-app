...
import { ChevronLeft, HardHat, CloudRain, Clock, Loader2, FileCheck, Images, Plus, Minus, Trash2, CloudLightning } from 'lucide-react'
...
export default function NewDailyLog() {
  ...
  useEffect(() => {
    async function initForm() {
      ...
      // AUTO-WEATHER FETCH (Uses IP to get local forecast automatically)
      try {
        const res = await fetch('https://wttr.in/?format="%C, %t, Wind %w"')
        const text = await res.text()
        setWeather(text.replace(/"/g, '').trim())
      } catch (err) {
        setWeather('') // Fallback to empty if offline
      }
  ...
  const saveLog = async (isFinal: boolean) => {
    setSaving(true)
    const payload = {
      project_id: id, log_date: date, weather, work_performed: workPerformed,
      manpower: getManpowerString(), status: isFinal ? 'Final' : 'Draft',
      signature: isFinal ? signature : null, photo_urls: photos
    }
    const { error } = await supabase.from('daily_logs').insert([payload])
    if (!error) { router.push(`/projects/${id}/logs`); router.refresh(); } 
  ...
  return (
    <div className="max-w-4xl mx-auto p-4 md:p-8 bg-slate-950 min-h-screen font-sans text-slate-100 pb-40">
      <div className="mb-8 border-b-4 border-blue-600 pb-6 flex justify-between items-end">
        <div>
          <button onClick={() => router.back()} className="text-[10px] font-black uppercase text-slate-500 mb-4 hover:text-white flex items-center gap-1"><ChevronLeft size={12}/> Discard</button>
            ...
            {contacts.map(trade => (
              <div key={trade.id} className="bg-slate-950 border border-slate-800/50 p-4 rounded-2xl flex items-center justify-between">
                ...
                <div className="flex items-center gap-4 bg-slate-900 p-1 rounded-xl border border-slate-800">
                  ...
                  <button onClick={() => adjustCount(trade.id, 1)} className="w-10 h-10 rounded-lg bg-blue-600 text-white hover:bg-blue-500 shadow-lg shadow-blue-900/20"><Plus size={16} /></button>
                </div>
              </div>
            ))}
          </div>
      ...
      <div className="fixed bottom-0 left-0 w-full bg-slate-950/90 border-t border-slate-800 p-4 backdrop-blur-md z-50">
        <div className="max-w-4xl mx-auto flex gap-4">
          <button onClick={() => saveLog(false)} disabled={saving || uploading} className="flex-1 bg-slate-800 text-white font-black py-4 rounded-2xl text-[10px] uppercase tracking-widest transition-all hover:bg-slate-700">{saving ? <Loader2 size={16} className="animate-spin mx-auto"/> : 'Save Draft'}</button>
          <button onClick={() => { if(!signature) return alert("Sign before finalizing."); saveLog(true); }} disabled={saving || uploading} className="flex-1 bg-emerald-600 text-white font-black py-4 rounded-2xl text-[10px] uppercase tracking-widest shadow-xl shadow-emerald-900/20 transition-all hover:bg-emerald-500">Finalize & Sign</button>
        </div>
      </div>
    </div>
  )
