...
export default function NewDailyLog() {
  ...
  // MANPOWER COUNTER STATE
  // This stores { "contact_id": count }
  const [tradeCounts, setTradeCounts] = useState<Record<string, number>>({})

  useEffect(() => {
    async function initForm() {
      if (!id) return
      // Fetch the trades registered to this project
      const { data } = await supabase
        .from('project_contacts')
        .select('id, company, trade_role')
        .eq('project_id', id)
  ...
  const adjustCount = (contactId: string, amount: number) => {
    setTradeCounts(prev => ({
      ...prev,
      [contactId]: Math.max(0, (prev[contactId] || 0) + amount)
    }))
  }

  // Generate the text string for the database
  // Result: "3 Framers, 2 Drywallers, 1 Electrician"
  const getManpowerString = () => {
    return contacts
      .filter(c => tradeCounts[c.id] > 0)
      .map(c => `${tradeCounts[c.id]} ${c.company} (${c.trade_role})`)
      .join(', ')
  }
  ...
  const saveLog = async (isFinal: boolean) => {
    ...
    const payload = {
      ...
      manpower: getManpowerString(), // Built from the counters
      ...
      <div className="space-y-6">
        ...
        <div className="bg-slate-900/50 p-6 rounded-[32px] border border-slate-800">
            ...
            {contacts.map(trade => (
              <div key={trade.id} className="bg-slate-950 border border-slate-800/50 p-4 rounded-2xl flex items-center justify-between">
                ...
                <div className="flex items-center gap-4 bg-slate-900 p-1 rounded-xl border border-slate-800">
                  <button 
                    onClick={() => adjustCount(trade.id, -1)}
                    className="w-10 h-10 rounded-lg bg-slate-800 flex items-center justify-center text-slate-400 hover:text-red-500 transition-colors"
                  >
                    <Minus size={16} />
                  </button>
                  
                  <span className="text-xl font-black text-blue-400 w-6 text-center">
                    {tradeCounts[trade.id] || 0}
                  </span>

                  <button 
                    onClick={() => adjustCount(trade.id, 1)}
                    className="w-10 h-10 rounded-lg bg-blue-600 text-white flex items-center justify-center hover:bg-blue-500 transition-colors shadow-lg shadow-blue-900/20"
                  >
                    <Plus size={16} />
                  </button>
                </div>
          ...
          <div className="mt-4 p-4 bg-blue-600/5 border border-blue-500/20 rounded-xl">
            ...
            <p className="text-[10px] font-bold text-slate-400 italic">
              {getManpowerString() || "No workers selected"}
            </p>
          </div>
        </div>
        ...
        <div className="bg-slate-900/50 p-6 rounded-[32px] border border-slate-800">
          <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest mb-3 flex items-center gap-2">
            <PenTool size={14} className="text-blue-500"/> Signature
          </label>
